<?php include"include/header.php"; ?>
    <!--Mask-->
    <div class="view view-portfolio hm-black-strong">
        <div class="full-bg-img flex-center">
            <ul>
                <li>
                    <h1 class="h1-responsive wow fadeInDown" data-wow-delay="0.2s"><?php echo $lang['About_slider'] ?></h1></li>
                <li>
                    <p class="wow fadeInDown"><?php echo $lang['About_slider_sub'] ?></p>
                </li>
            </ul>
        </div>
    </div>
    <!--/.Mask-->
    <!-- Main container-->
    <div class="container">
        <!--Section: About-->
			<section class="section feature-box">
				<!--Section heading-->
				<h1 class="section-heading"><?php echo $lang['Portfolio_main_heading1'] ?></h1>
				<p class="section-description"><?php echo $lang['Portfolio_main_heading1_sub'] ?></p>
				<!--First row-->
				<div class="row features-big">
					<!--First column-->
					<div class="col-md-4 mb-r">
						<i class="fa fa-bars blue-text"></i>
						<h4 class="feature-title"><?php echo $lang['Portfolio_main_subheading1'] ?></h4>
						<p class="grey-text"><?php echo $lang['Portfolio_main_content1'] ?></p>
					</div>
					<!--/First column-->
					<!--Second column-->
					<div class="col-md-4 mb-r">
						<i class="fa fa-globe green-text"></i>
						<h4 class="feature-title"><?php echo $lang['Portfolio_main_subheading2'] ?></h4>
						<p class="grey-text"><?php echo $lang['Portfolio_main_content2'] ?></p>
					</div>
					<!--/Second column-->
					<!--Third column-->
					<div class="col-md-4 mb-r">
						<i class="fa fa-clone red-text"></i>
						<h4 class="feature-title"><?php echo $lang['Portfolio_main_subheading3'] ?></h4>
						<p class="grey-text"<?php echo $lang['Portfolio_main_content3'] ?></p>
					</div>
					<!--/Third column-->
				</div>
				<!--/First row-->
			</section>
			<!--/Section: Features v.1-->
<!--Projects section v.1-->
<section class="section section-port">
    <!--Section heading-->
    <h1 class="section-heading"><?php echo $lang['Portfolio_about_heading2'] ?></h1>
    <!--Section description-->
    <p class="section-description"><?php echo $lang['Portfolio_about_heading2_sub'] ?></p>
    <!--First row-->
    <div class="row">
        <!--First column-->
        <div class="col-md-12 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/flazy.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                         <a href="" class="red-text"><h5><i class="fa fa-money"></i><?php echo $lang['Portfolio_category_communicate'] ?></h5></a>
                        <h3>Flazy</h3>
                        <p><?php echo $lang['Portfolio_proj_desc1'] ?></p>
                        <a href="http://flazy.org/" class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/First column-->
        <!--Second column-->
        <div class="col-md-6 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/techwhiskers.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                        <a href="" class="teal-text"><h5><i class="fa fa-pie-chart"></i><?php echo $lang['Portfolio_category_marketing'] ?></h5></a>
                        <h3>TechWhiskers</h3>
                        <p><?php echo $lang['Portfolio_proj_desc2'] ?></p>
                        <a href="https://www.facebook.com/techwhiskers/" class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/Second column-->
        <!--Third column-->
        <div class="col-md-6 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/minecms.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                        <a href="" class="cyan-text"><h5><i class="fa fa-desktop"></i><?php echo $lang['Portfolio_category_software'] ?></h5></a>
                        <h3>Mine CMS</h3>
                        <p><?php echo $lang['Portfolio_proj_desc3'] ?></p>
                        <a href="https://github.com/MGKNeT/Mine-CMS"class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/Third column-->
		 <!--Second column-->
        <div class="col-md-6 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/kartene.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                        <a href="" class="teal-text"><h5><i class="fa fa-pie-chart"></i><?php echo $lang['Portfolio_category_marketing'] ?></h5></a>
                        <h3>Kartene-izvozva-remont</h3>
                        <p><?php echo $lang['Portfolio_proj_desc4'] ?></p>
                        <a href="https://kartene-izvozva-remont.com/" class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/Second column-->
        <!--Third column-->
        <div class="col-md-6 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/groweuropesales.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                        <a href="" class="cyan-text"><h5><i class="fa fa-desktop"></i><?php echo $lang['Portfolio_category_software'] ?></h5></a>
                        <h3>Grow Europe Sales</h3>
                        <p><?php echo $lang['Portfolio_proj_desc5'] ?></p>
                        <a href="http://groweuropesales.co.uk/"class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/Third column-->
        <!--First column-->
        <div class="col-md-12 mb-r">
            <!--Card-->
            <div class="card-overlay" style="background-image: url('https://mgknet.com/images/projects/lyos.png')">
                <!--Content-->
                <div class="white-text text-center">
                    <div class="card-block">
                        <a href="" class="red-text"><h5><i class="fa fa-cloud"></i><?php echo $lang['Portfolio_category_communicate'] ?></h5></a>
                        <h3>LyOS</h3>
                        <p><?php echo $lang['Portfolio_proj_desc6'] ?></p>
                        <a href="#" class="btn btn-lg btn-outline-white"><i class="fa fa-clone left"></i><?php echo $lang['Portfolio_btn_view'] ?></a>
                    </div>
                </div>
            </div>
            <!--/.Card-->
        </div>
        <!--/First column-->
    </div>
    <!--/First row-->
</section>
<!--/Projects section v.1-->
    </div>
    <!--/ Main container-->
<?php include"include/footer.php"; ?>